...
--2
     ...
     IF @sDate <= @BaseDate 
     BEGIN
     SET @sDate = CONVERT( CHAR(8), DATEADD(D, 1, CONVERT( DATETIME, @BaseDate ) ), 112 )
     END
       ...
       FROM (
              ...
              AND A.StuffDate BETWEEN @sDate  AND @eDate
              AND ( ( @nChkCustom = 0 )
              OR ( @nChkCustom = 1 AND C.CustomID = @CustomID ) )
              AND ( ( @nChkArticle = 0 )
              OR ( @nChkArticle = 1 AND C.ArticleID = @ArticleID ) )
              AND ( ( @nChkOrderID = 0 )
              OR ( @nChkOrderID = 1 AND C.OrderID = @OrderID ) )
                 Union All
              ...
              AND A.OrderID = C.OrderID
              AND A.OutDate BETWEEN @sDate  AND @eDate
              AND ( ( @nChkCustom = 0 )
              OR ( @nChkCustom = 1 AND C.CustomID = @CustomID ) )
              AND ( ( @nChkArticle = 0 )
              OR ( @nChkArticle = 1 AND C.ArticleID = @ArticleID ) )
              AND ( ( @nChkOrderID = 0 )
              OR ( @nChkOrderID = 1 AND C.OrderID = @OrderID ) )              Union All
              ...
              AND ( ( @nChkOrderID = 0 )
              OR ( @nChkOrderID = 1 AND C.OrderID = @OrderID ) )
             group by a.orderid,a.outdate,a.outseq)A
...
--4
     ...
     SELECT DISTINCT A.OrderID, IODate = ' ', Cls = '0', OrderSeq=888, StuffRoll=0, StuffQty = ISNULL( A.StockQty +  A.StuffINQty -  A.OutQty, 0 ),
       ...
       INTO #Stock
       FROM dbo.DSF06(@sDate, @nChkCustom, @CustomID, @nChkArticle, @ArticleID, @nChkOrderID, @OrderID )  A
