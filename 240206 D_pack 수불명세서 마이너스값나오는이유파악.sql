...
--2
     ...
     FROM DS30C
       ...
       FROM (
		  ...
		  select * from #subul
		  select * from #subul where stuffqty<0
...
--4
     ...
     SELECT DISTINCT A.OrderID, IODate = ' ', Cls = '0', OrderSeq=888, StuffRoll=0, StuffQty = ISNULL( A.StockQty +  A.StuffINQty -  A.OutQty, 0 ),
	   ...
	   select * from #Stock where stuffqty<0
...
--6
         select  Orderno,kCustom,Article,Color,orderid,
                  ...
                  case when cls = 0 then StuffQty else 0 end PrevQty,			--!!!!!이부분도봐야함
                  substring(IODate,5,2) + '-' + substring(IODate,7,2) IODate,
                  case when cls <> 0   then StuffQty else 0 end StuffQty,		--!!!!!!이부분보기
                  OutQty,OutRealQty,
